...
export default function Messages({ conversations, activeMessages, activeRecipient, allowedRecipients }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Messages', href: '/messages' }]}>
            ...
            <div className="flex h-[calc(100vh-64px)] w-full flex-col bg-white dark:bg-zinc-950 transition-colors duration-200">
                <div className="flex h-full w-full overflow-hidden border-t border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                    ...
                    <div
                        className={cn(
                            ...
                            'w-full flex-col border-r border-slate-200/60 dark:border-slate-800 bg-white dark:bg-zinc-950 md:w-80 lg:w-[400px] shrink-0 transition-colors'
                        )}
                    >
                        <div className="p-4 sm:p-5 shrink-0 bg-white dark:bg-zinc-950 z-10 transition-colors">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight transition-colors">Chats</h2>
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => setIsComposeOpen(true)}
                                    className="h-10 w-10 rounded-full bg-indigo-50 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-500/30 transition-colors"
                                >
                        ...
                        <div className="custom-scrollbar flex-1 overflow-y-auto px-3 pb-3">
                            {filteredConvos.length === 0 ? (
                                <div className="p-8 text-center text-sm text-slate-400 dark:text-slate-500">No conversations found.</div>
                            ) : (
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    isActive ? 'bg-indigo-50/80 dark:bg-indigo-500/20 ring-1 ring-indigo-100 dark:ring-indigo-500/30 shadow-sm' : 'hover:bg-slate-50 dark:hover:bg-zinc-900'
                                                )}
                                            >
                                                <div className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-100 to-indigo-200 dark:from-indigo-500/30 dark:to-indigo-600/30 text-lg font-bold text-indigo-700 dark:text-indigo-400 shadow-inner">
                                                ...
                                                <div className="flex-1 overflow-hidden">
                                                    <div className="flex items-baseline justify-between">
                                                        <span className={cn(
                                                            ...
                                                            !convo.last_message.is_read && !isActive ? "font-bold text-slate-900 dark:text-white" : "font-semibold text-slate-800 dark:text-slate-200"
                                                        )}>
                                                            {fullName}
                                                        </span>
                                                        <span className={cn(
                                                            "text-xs transition-colors",
                                                            !convo.last_message.is_read && !isActive ? "font-bold text-indigo-600 dark:text-indigo-400" : "text-slate-400 dark:text-slate-500"
                                                        )}>
                                                    ...
                                                    <div className={cn(
                                                        ...
                                                        !convo.last_message.is_read && !isActive ? "font-semibold text-indigo-600 dark:text-indigo-400" : "text-slate-500 dark:text-slate-400"
                                                    )}>
                                ...
                                <div className="shrink-0 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 bg-white/95 dark:bg-zinc-950/95 px-4 sm:px-6 py-3 shadow-sm z-10 backdrop-blur-md transition-colors">
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            className="-ml-2 rounded-full p-1.5 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-zinc-900 md:hidden transition-colors"
                                        >
                                            <ChevronLeft className="h-6 w-6" />
                                        </button>
                                        <div className="relative flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 text-[15px] font-bold text-white shadow-md ring-2 ring-white dark:ring-zinc-950 transition-all">
                                        ...
                                        <div className="ml-1">
                                            ...
                                            <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 transition-colors">
                                                {activeRecipient.is_online ? <span className="text-emerald-500 dark:text-emerald-400">Online</span> : 'Offline'} <span className="mx-1">•</span> {activeRecipient.role}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-6 z-10 relative custom-scrollbar">
                                    {messages.length === 0 ? (
                                        <div className="flex h-full flex-col items-center justify-center text-sm text-slate-400 dark:text-slate-500">
                                            ...
                                            <p className="mt-1 font-semibold text-indigo-500 dark:text-indigo-400 uppercase tracking-widest text-[11px] bg-indigo-50 dark:bg-indigo-500/10 px-3 py-1 rounded-full transition-colors">{activeRecipient.role}</p>
                                            ...
                                            return (
                                                <div key={msg.id} className={cn('flex w-full flex-col', isMe ? 'items-end' : 'items-start')}>
                                                        ...
                                                        <div className={cn('mb-1 flex max-w-[85%] flex-col', isMe ? 'items-end' : 'items-start')}>
                                                            {isImg ? (
                                                                <button
                                                                    ...
                                                                    className="overflow-hidden rounded-2xl border border-slate-200/60 dark:border-slate-800 shadow-sm cursor-zoom-in group bg-white dark:bg-zinc-900 p-1 transition-colors"
                                                                >
                                                                ...
                                                                <a
                                                                    ...
                                                                    className={cn(
                                                                        ...
                                                                        isMe ? "bg-indigo-600 dark:bg-indigo-500 text-white rounded-br-[4px]" : "bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 rounded-bl-[4px]"
                                                                    )}
                                                                >
                                                                    <div className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-xl", isMe ? "bg-indigo-500/50 dark:bg-indigo-400/30" : "bg-slate-100 dark:bg-zinc-800 shadow-inner")}>
                                                                        <FileText className={cn("h-5 w-5", isMe ? "text-indigo-50" : "text-indigo-600 dark:text-indigo-400")} />
                                                                    </div>
                                                    ...
                                                    {msg.body && (
                                                        <div
                                                            className={cn(
                                                                'max-w-[75%] px-4 py-2.5 text-[15px] leading-relaxed whitespace-pre-wrap shadow-sm font-medium transition-colors',
                                                                isMe
                                                                    ? 'bg-indigo-600 dark:bg-indigo-600 text-white rounded-2xl rounded-br-[4px]'
                                                                    : 'bg-white dark:bg-zinc-900 border border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-100 rounded-2xl rounded-bl-[4px]',
                                                    ...
                                                    <span className={cn(
                                                        "text-[10px] font-medium text-slate-400 dark:text-slate-500 mt-1 mx-1 transition-colors",
                                ...
                                <div className="shrink-0 w-full bg-white dark:bg-zinc-950 px-3 pb-4 pt-3 sm:px-6 sm:pb-6 z-20 relative transition-colors">
                                    ...
                                    <form onSubmit={handleSendReply} className="flex items-end gap-2 relative w-full">
                                        ...
                                        <Button
                                            ...
                                            className="mb-1 shrink-0 h-11 w-11 rounded-full text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-zinc-900 transition-colors"
                                        >
                ...
                <Transition appear show={isComposeOpen} as={Fragment}>
                    <Dialog as="div" className="relative z-50" onClose={() => setIsComposeOpen(false)}>
                        <div className="fixed inset-0 bg-slate-900/40 dark:bg-black/60 backdrop-blur-sm transition-colors" />
                        <div className="fixed inset-0 overflow-y-auto">
                            <div className="flex min-h-full items-center justify-center p-4">
                                <Dialog.Panel className="w-full max-w-md overflow-hidden rounded-2xl bg-white dark:bg-zinc-950 shadow-2xl flex flex-col h-[500px] border border-slate-200 dark:border-slate-800 transition-colors">

                                    {/* Header & Search */}
                                    <div className="flex-none bg-white dark:bg-zinc-950 z-10 transition-colors">
                                        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-start transition-colors">
                                            <div>
                                                <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight transition-colors">New Message</h3>
                                                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-1 transition-colors">Select a contact to start chatting</p>
                                            </div>
                                            <Button variant="ghost" size="icon" onClick={() => setIsComposeOpen(false)} className="rounded-full bg-slate-50 dark:bg-zinc-900 text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white h-8 w-8 -mt-1 -mr-1 transition-colors">
                                                <X className="h-4 w-4" />
                                            </Button>
                                        </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto px-3 pb-3 custom-scrollbar">
                                        {filteredContacts.length === 0 ? (
                                            <div className="p-6 text-center text-sm text-slate-400 dark:text-slate-500 font-medium transition-colors">No contacts found matching "{contactSearch}"</div>
                                        ) : (
                                            <div className="flex flex-col gap-1.5 mt-2">
                                                {filteredContacts.map(u => (
                                                    <button
                                                        ...
                                                        className="flex items-center gap-3 w-full p-2.5 rounded-xl text-left transition-all hover:bg-slate-50 dark:hover:bg-zinc-900 bg-transparent text-slate-800 dark:text-slate-200"
                                                    >
                                                        <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-sm font-bold shadow-sm bg-gradient-to-br from-indigo-100 to-indigo-200 dark:from-indigo-500/20 dark:to-indigo-600/20 text-indigo-700 dark:text-indigo-400 border border-white dark:border-zinc-950 transition-colors">
                                                            {getInitials(u.first_name, u.last_name)}
                                                        </div>
                                                        <div className="flex flex-col overflow-hidden">
                                                            <span className="truncate text-[15px] font-bold leading-tight">{u.first_name} {u.last_name}</span>
                                                            <span className="text-[11px] font-bold uppercase tracking-wider mt-0.5 text-slate-400 dark:text-slate-500 transition-colors">{u.role}</span>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>

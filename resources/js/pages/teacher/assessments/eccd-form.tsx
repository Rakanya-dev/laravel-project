            ...
            <div className="w-full space-y-6 p-4 sm:p-6 lg:p-8 transition-colors duration-200">
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
                    ...
                    <div className="space-y-6 lg:col-span-4 xl:col-span-3">
                        <div className="sticky top-8 space-y-6">
                            ...
                            <div className="overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-zinc-900 shadow-sm transition-colors">
                                <div className="h-12 bg-gradient-to-r from-emerald-500 to-teal-600 dark:from-emerald-600 dark:to-teal-700"></div>
                                ...
                                <div className="px-6 pb-6 pt-0">
                                    ...
                                    {/* Progress Bar */}
                                    <div className="mt-6 rounded-2xl bg-slate-50 dark:bg-zinc-950 p-4 border border-slate-100 dark:border-slate-800 transition-colors">
                                        ...
                                        <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-zinc-800 transition-colors">
                                            <div
                                                className={`h-full transition-all duration-500 ease-out ${progressPercentage === 100 ? 'bg-emerald-500 dark:bg-emerald-400' : 'bg-indigo-500 dark:bg-indigo-400'
                                                    }`}
                            ...
                            {!isReadOnly && (
                                <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-zinc-900 p-4 shadow-sm transition-colors">
                                    <div className="space-y-3">
                                        <Button
                                            className="h-12 w-full text-base font-bold shadow-md transition-all bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-600 dark:hover:bg-emerald-500 text-white rounded-xl"
                                            onClick={() => {
                                        ...
                                        >
                                            <Save className={`mr-2 size-5 ${isDirty ? 'text-indigo-500' : 'text-slate-400'}`} />
                                            {isDirty ? 'Save Progress' : 'Changes Saved'}
                                        </Button>
                                    </div>
                                    {scoredDomains !== totalDomains && (
                                        <p className="mt-3 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest transition-colors">
                                            Score all domains to enable submission
                                        </p>
                                    )}
                                </div>
                            )}
                            ...
                            {isReadOnly && (
                                <Alert className="border-indigo-200 dark:border-indigo-900/50 bg-indigo-50 dark:bg-indigo-500/5 text-indigo-800 dark:text-indigo-300 shadow-sm rounded-2xl p-4 transition-colors">
                    ...
                    <div className="lg:col-span-8 xl:col-span-9 transition-colors">
                        <div className="overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-zinc-900 shadow-sm transition-colors">
                            <div className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-zinc-900 p-6 sm:p-8 flex items-center gap-4 transition-colors">
                                <div className="flex size-12 items-center justify-center rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 transition-colors">
                                    <ClipboardList className="size-6" />
                                </div>
                                <div>
                                    <h2 className="text-xl font-extrabold text-slate-900 dark:text-white transition-colors">Domain Scoring</h2>
                                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400 transition-colors">Enter the raw score observed for each developmental domain.</p>
                                </div>
                            </div>
                                    ...
                                    <TableBody className="divide-y divide-slate-100 dark:divide-slate-800 transition-colors">
                                        {data.scores.map((score: any, index: number) => {
                                            ...
                                            return (
                                                <TableRow
                                                    key={score.id}
                                                    className={`transition-colors hover:bg-slate-50/50 dark:hover:bg-zinc-800/50 ${hasScore ? 'bg-indigo-50/10 dark:bg-indigo-500/5' : ''
                                                        }`}
                                                >
                                                            ...
                                                            <div className="relative">
                                                                <Input
                                                                    ...
                                                                    className={`h-12 w-32 text-center text-xl font-black shadow-sm transition-all focus-visible:ring-4 dark:bg-zinc-950 ${hasScore
                                                                        ? isMaxed
                                                                            ? 'border-emerald-400 dark:border-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-900 dark:text-emerald-400 focus-visible:ring-emerald-500/20'

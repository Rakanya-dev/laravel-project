...
export function ParentChatTab(props: Props) {
    ...
    return (
        <div className="flex h-[650px] w-full overflow-hidden bg-white rounded-b-2xl border border-slate-200 shadow-sm border-t-0">

            {/* ================= LEFT: CONVERSATION LIST ================= */}
            <div className="hidden md:flex w-80 lg:w-[350px] flex-col border-r border-slate-200/60 bg-white shrink-0">
                <div className="p-4 shrink-0 bg-white z-10 border-b border-slate-50">
                ...
                <div className="custom-scrollbar flex-1 overflow-y-auto px-3 py-2">
                    {filteredConvos.length === 0 ? (
                        <div className="p-8 text-center text-sm text-slate-400 font-medium">No contacts found.</div>
                    ) : (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 overflow-hidden">
                                            ...
                                            <div className={cn(
                                                ...
                                                isUnread ? "font-bold text-indigo-600" : "text-slate-500")}>
                                                {convo.last_message || convo.contact_role}
                                            </div>
                                            {isUnread && (
                                                <div className="h-2.5 w-2.5 rounded-full bg-indigo-600 shadow-sm ml-2"></div>
                                            )}
                                        </div>
                                    </button>
                                );
                    ...
                    <>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="ml-1">
                                    ...
                                    <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">
                                        {activeContact.is_online ? <span className="text-emerald-500">Online</span> : 'Offline'} <span className="mx-1">•</span> {activeContact.contact_role}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 space-y-4 overflow-y-auto p-4 sm:p-6 z-10 relative custom-scrollbar">
                            {isLoading && messages.length === 0 ? (
                                <div className="flex h-full items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-indigo-500" /></div>
                            ) : messages.length === 0 ? (
                                <div className="flex h-full flex-col items-center justify-center text-sm text-slate-400">
                                    ...
                                    <h3 className="text-xl font-bold text-slate-800">{activeContact.contact_name}</h3>
                                    <p className="mt-1 font-semibold text-indigo-500 uppercase tracking-widest text-[11px] bg-indigo-50 px-3 py-1 rounded-full">{activeContact.contact_role}</p>
                                        ...
                                        <div key={msg.id} className={cn('flex w-full flex-col', isMe ? 'items-end' : 'items-start')}>
                                            ...
                                            {msg.attachment_url && (
                                                <div className={cn('mb-1 flex max-w-[85%] flex-col', isMe ? 'items-end' : 'items-start')}>
                                                    {isImg ? (
                                                        <button type="button" onClick={() => openLightbox(msg.id)} className="overflow-hidden rounded-2xl border border-slate-200/60 shadow-sm cursor-zoom-in group bg-white p-1">
                                                        ...
                                                        <a href={msg.attachment_url} download={fileName} target="_blank" rel="noopener noreferrer" className={cn("flex items-center gap-3 rounded-2xl p-3 shadow-sm transition-all hover:scale-[1.02]", isMe ? "bg-indigo-600 text-white rounded-br-[4px]" : "bg-white border border-slate-200 text-slate-900 rounded-bl-[4px]")}>
                                            ...
                                            {/* Text Body */}
                                            {msg.body && (
                                                <div className={cn('max-w-[75%] px-4 py-2.5 text-[15px] leading-relaxed whitespace-pre-wrap shadow-sm font-medium', isMe ? 'bg-indigo-600 text-white rounded-2xl rounded-br-[4px]' : 'bg-white border border-slate-100 text-slate-800 rounded-2xl rounded-bl-[4px]', msg.is_optimistic && 'opacity-70')}>
                                                    {msg.body}
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div className="shrink-0 w-full bg-white px-3 pb-4 pt-3 sm:px-6 sm:pb-6 z-20 relative">
                            {showEmojiPicker && (
                                <div ref={pickerRef} className="absolute bottom-full mb-4 left-6 z-50 shadow-2xl rounded-2xl overflow-hidden border border-slate-200">
                                    <EmojiPicker onEmojiClick={onEmojiClick} autoFocusSearch={false} theme={Theme.LIGHT} searchDisabled={false} skinTonesDisabled={true} height={350} width={320} previewConfig={{ showPreview: false }} />
                                </div>
                            )}
                            ...
                            <form onSubmit={handleSendReply} className="flex items-end gap-2 relative w-full">
                                ...
                                <Button type="button" variant="ghost" size="icon" onClick={() => fileInputRef.current?.click()} className="mb-1 shrink-0 h-11 w-11 rounded-full text-indigo-600 hover:bg-indigo-50 transition-colors">
                                ...
                                <div className="flex-1 flex items-end bg-slate-50 transition-all duration-300 border border-slate-200 rounded-3xl focus-within:bg-white focus-within:border-indigo-400 focus-within:shadow-md focus-within:ring-4 focus-within:ring-indigo-500/10 p-1.5">
                                    <Button type="button" variant="ghost" size="icon" onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={cn("mb-0.5 shrink-0 h-9 w-9 rounded-full ml-1 transition-colors", showEmojiPicker ? "text-indigo-600 bg-indigo-50" : "text-slate-400 hover:text-indigo-600 hover:bg-slate-200")}>
                                    ...
                                    <Button type="submit" size="icon" variant="ghost" disabled={(!replyText.trim() && !attachment) || isSending} className={cn('mb-0.5 shrink-0 h-9 w-9 rounded-full transition-all mr-1', (replyText.trim() || attachment) ? 'bg-indigo-600 text-white shadow-md hover:bg-indigo-700 hover:scale-105' : 'text-slate-300 cursor-not-allowed hover:bg-transparent')}>
